...
// Modifications by Charley Jones, PMP
// For SMTBoards.Com
// Add Turbo Cylon Modes for faster scanning.
// 0c, 0d, 0e...
// Tigthen up code to make some room.
...
typedef enum {
	...
	MODE_0c, // turbo cylon scanner
	MODE_0d, // single direction cylon scan
	MODE_0e, // other direction cylon scan
...
#define DEF_CYLON_SCAN_DELAY 30
#define DEF_CYLON_SCAN_TURBO 10

void cylon(unsigned char cylon_style) {

	unsigned char CYLON_SCAN_DELAY =  DEF_CYLON_SCAN_DELAY;
        if (cylon_style>=3) {cylon_style-=3; CYLON_SCAN_DELAY=DEF_CYLON_SCAN_TURBO;} 
...
void main(void) {
	...
	switch(mode) {
		...
		case MODE_0c: // turbo traditional (back & forth) cylon scanner
		case MODE_0d: // turbo single direction cylon scanner
		case MODE_0e: // turbo other direction cylon scanner
			cylon(mode);
			break;

		case MODE_1:  // (2) 2 glowing pig eyes
		case MODE_1a: // (1) single glowing pig eye
		case MODE_1b: // (0) single (random) glowing pig eye
			pig_eyes(MODE_1b-mode);
			break;
